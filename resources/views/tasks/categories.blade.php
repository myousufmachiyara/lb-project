...
@section('title', 'Tasks | Categories')
  ...
  <div class="row">
    <div class="col">
                ...
                @foreach ($taskCat as $item)
                  <tr>
                    ...
                    <td>
                        ...
                        <form action="{{ route('task-categories.destroy', $item->id) }}" method="POST" style="display:inline;">
                            @csrf
                            @method('DELETE')
      ...
      <div id="addModal" class="modal-block modal-block-primary mfp-hide">
        <section class="card">
          <form method="post" action="{{ route('task-categories.store') }}" enctype="multipart/form-data" onkeydown="return event.key != 'Enter';">
            @csrf
            <header class="card-header">
                <h2 class="card-title">New Task Category</h2>
            </header>
            <div class="card-body">
                ...
                <div class="form-group mb-3">
                    ...
                    <input type="text" class="form-control" placeholder="Stage Shortcode" name="code" required>
                </div>
            </div>
      ...
      <div id="updateModal" class="modal-block modal-block-primary mfp-hide">
        <section class="card">
          <form method="post" action="" enctype="multipart/form-data" onkeydown="return event.key != 'Enter';">
            @csrf
            <header class="card-header">
                <h2 class="card-title">Update Task Category</h2>
            </header>
            <div class="card-body">
                <div class="form-group">
                    <input type="number" class="form-control" id="update_category_id" required disabled>
                </div>
                <div class="form-group">
                    <label>Project Stage Name<span style="color: red;"><strong>*</strong></span></label>
                    <input type="text" class="form-control" id="update_category_name" placeholder="Name" name="name" required>
  ...
  <script>
    $(document).ready(function () {
      $('.editStatusBtn').on('click', function () {
        ...
        $.ajax({
          url: `/task-categories/${statusId}/json`,
          method: 'GET',
          success: function (data) {
            $('#update_category_id').val(data.id);
            $('#update_category_name').val(data.name);
            $('#update_category_code').val(data.code);
            $('#category_id').val(data.id);

            // Set form action dynamically
            const formAction = `/task-categories/${data.id}`;
